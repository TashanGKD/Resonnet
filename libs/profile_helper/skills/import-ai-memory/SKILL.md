...
**一句话总结：只补 AI 没给答案的，不要确认 AI 已经给了答案的。**
...
| **需补充** | AI 标注 ❌ 记忆不足，或完全未提及该字段 | 与 collect-basic-info 一致的问法补充 |
...
- 学术合作缺失 → ask_choice: "你的学术合作情况？" options: [实验室内/跨机构/跨学科/都有/独立工作]
...
- question: "你的所在机构是「[机构名]」，是否正确？"
- options:
  - {id:"yes", label:"正确"}
  - {id:"modify", label:"需要修改", text_prompt:"请输入正确的机构名称"}
  （text_prompt 会让前端在「需要修改」按钮右侧直接显示内联输入框，用户无需二次点击）
...
- **基础信息完整，量表维度为空**（最常见情况）：
  ...
  - options:
    ...
    - {id: "done", label: "暂时到这里"}
  若选「推断」→ 调用 read_skill("infer-profile-dimensions")

- **基础信息仍有空白**：
  调用 ask_choice：
  - question: "AI 记忆已导入！不过还有一些字段未能获取，建议继续补充。"
  - options: [{id: "continue", label: "继续补充"}, {id: "later", label: "暂时到这里"}]
  若选「继续」→ 回到 collect-basic-info 补充空白字段

- **画像已相当完整**：
  调用 show_actions：
  - message: "AI 记忆已成功导入！画像相当完整。"
  - buttons: [{id: "view", label: "查看完整画像", href: "/profile-helper/profile", style: "primary"}, {id: "scale", label: "量表校对", href: "/profile-helper/scales", style: "secondary"}]
